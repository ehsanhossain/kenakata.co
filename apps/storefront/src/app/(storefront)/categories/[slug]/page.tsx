  ...
  return (
    <div>
      ...
      <div className="bg-neutral-50 border-b border-neutral-100">
        <div className="container-page py-3">
          <nav className="flex items-center gap-2 text-sm text-neutral-500">
            ...
            <span>/</span>
            <span className="text-brand-charcoal font-medium">Electronics</span>
          </nav>
        </div>
      </div>
      ...
      <div className="container-page py-6 sm:py-8">
        {/* Page header */}
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-brand-charcoal">Electronics</h1>
          <p className="text-sm text-neutral-500 mt-1">{filteredProducts.length} products found</p>
        </div>
        ...
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-[264px] shrink-0">
            <div className="sticky top-[140px] space-y-6">
                  ...
                  {brands.map(brand => (
                    <label key={brand.id} className="flex items-center gap-2.5 cursor-pointer group">
                      <input
                        ...
                        className="w-4 h-4 rounded border-neutral-300 text-brand-blue focus:ring-brand-blue/30"
                      />
                      <span className="text-sm text-neutral-700 group-hover:text-brand-charcoal">{brand.name}</span>
                      <span className="ml-auto text-xs text-neutral-400">({brand.productCount})</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-neutral-100 p-4">
                ...
                {[4, 3, 2, 1].map(stars => (
                  <label key={stars} className="flex items-center gap-2 py-1.5 cursor-pointer">
                    <input type="radio" name="rating" className="w-4 h-4 text-brand-blue focus:ring-brand-blue/30" />
              ...
              {/* Availability */}
              <div className="bg-white rounded-lg border border-neutral-100 p-4">
                <h3 className="text-sm font-semibold text-brand-charcoal mb-3">Availability</h3>
                <label className="flex items-center gap-2.5 cursor-pointer">
                  <input type="checkbox" defaultChecked className="w-4 h-4 rounded border-neutral-300 text-brand-blue focus:ring-brand-blue/30" />
          ...
          <div className="flex-1 min-w-0">
            {/* Sort & View Toolbar */}
            <div className="flex items-center justify-between gap-3 pb-4 border-b border-neutral-100 mb-6 sticky top-[120px] bg-white z-10 -mx-4 px-4 sm:mx-0 sm:px-0 py-2">
              ...
              {selectedBrands.length > 0 && (
                <div className="hidden sm:flex items-center gap-2 flex-wrap">
                  ...
                  <button onClick={() => setSelectedBrands([])} className="text-xs text-neutral-500 hover:text-semantic-danger transition-colors">
                    Clear all
                  </button>
                </div>
              )}
              ...
              <div className="flex items-center gap-3 ml-auto">
                {/* View mode toggle */}
                <div className="hidden sm:flex items-center bg-neutral-50 rounded-lg p-0.5">
                  <button
                    onClick={() => setViewMode('grid')}
                    className={`p-1.5 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-white shadow-sm text-brand-blue' : 'text-neutral-400'}`}
                  >
                    <Grid3X3 className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => setViewMode('list')}
                    className={`p-1.5 rounded-md transition-colors ${viewMode === 'list' ? 'bg-white shadow-sm text-brand-blue' : 'text-neutral-400'}`}
                  >
                ...
                <div className="relative">
                  <select
                    ...
                    className="appearance-none bg-neutral-50 border border-neutral-200 rounded-lg text-sm pl-3 pr-8 py-2 focus:ring-0 focus:border-brand-blue cursor-pointer"
                  >
                    {sortOptions.map(opt => (
            ...
            <div className="flex items-center justify-center gap-2 mt-10">
              ...
              <button className="w-9 h-9 rounded-lg bg-brand-blue text-white text-sm font-medium">1</button>
              <button className="w-9 h-9 rounded-lg text-neutral-600 text-sm hover:bg-neutral-50 transition-colors">2</button>
              <button className="w-9 h-9 rounded-lg text-neutral-600 text-sm hover:bg-neutral-50 transition-colors">3</button>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl max-h-[85vh] overflow-y-auto lg:hidden animate-slide-up safe-bottom">
            <div className="sticky top-0 bg-white border-b border-neutral-100 px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => { setSelectedBrands([]); }} className="text-sm text-brand-blue">Clear all</button>
            ...
            <div className="sticky bottom-0 bg-white border-t border-neutral-100 p-4 flex gap-3 safe-bottom">
