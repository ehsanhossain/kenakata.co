...
export default function OrdersPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-brand-charcoal">My Orders</h1>
          ...
          <Link key={order.id} href={`/account/orders/${order.id}`}
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-brand-charcoal group-hover:text-brand-blue transition-colors">
                  {order.id}
                </p>
                <p className="text-xs text-neutral-500 mt-0.5">{order.date} · {order.items} item{order.items > 1 ? 's' : ''}</p>
                <p className="text-xs text-neutral-400 mt-0.5 line-clamp-1">{order.itemNames}</p>
              </div>
            </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-neutral-500">No orders found matching your filters.</p>
        </div>
      )}
    </div>
  );
