...
import {
  ShoppingBag, Plus, MagnifyingGlass, CheckCircle, Clock, WarningCircle, PencilSimple, ArrowSquareOut, ArrowRight
...
export default function MerchantProductsPage() {
 ...
 return (
 <MerchantShell
 title="Storefront Catalog & Inventory"
 subtitle="Manage your products, live stock counts, and check admin moderation status"
 actions={
 <Link
 href="/products/new"
 className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-action-primary text-content-primary font-bold text-xs shadow-lg shadow-action-primary/10 hover:bg-success transition-all"
 >
 ...
 <span className="text-[10px] text-content-tertiary font-mono">SKU: {prod.sku}</span>
 </div>
 </div>
 </td>
 <td className="px-6 py-4 text-content-secondary">{prod.category}</td>
 <td className="px-6 py-4">
 <p className="font-mono font-bold text-content-primary">{prod.price}</p>
 {prod.compareAt && (
 <p className="text-[10px] font-mono text-content-tertiary line-through">{prod.compareAt}</p>
 )}
 </td>
 <td className="px-6 py-4 font-mono font-semibold text-content-primary">{prod.stock} units</td>
 <td className="px-6 py-4">
 {prod.status === 'APPROVED' ? (
 <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold bg-success-surface text-success border border-success/20">
 <CheckCircle className="w-3.5 h-3.5" /> Live on Storefront
 </span>
 ) : (
 <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-semibold bg-warning-surface text-warning border border-warning/20">
