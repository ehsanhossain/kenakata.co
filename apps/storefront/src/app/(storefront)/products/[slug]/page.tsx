...
import {
  Star, Heart, ShareNetwork, ShoppingCart, Lightning, CaretRight, CaretDown, ShieldCheck, Truck, ArrowCounterClockwise, CreditCard, Minus, Plus, Check, MapPin, ShoppingBag
...
function GalleryPlaceholder({ product }: { product: Product }) {
 ...
 <ShoppingBag className="w-24 h-24 text-content-tertiary/40" />
 </div>
 ...
 <div
 key={i}
 className={`w-1.5 h-1.5 rounded-full ${i === selected ? 'bg-action-primary' : 'bg-white/60'}`}
 />
 ))}
 </div>
 </div>
 ...
 ${selected === i ? 'border-border-brand shadow-focus' : 'border-transparent hover:border-border'}`}
 >
 <div
...
export default function ProductDetailPage() {
 ...
 <p className="text-sm text-content-tertiary mb-6">The product you&apos;re looking for doesn&apos;t exist.</p>
 <Link href="/" className="btn-primary">Back to House</Link>
 </div>
 );
 }
 ...
 <Link href="/" className="hover:text-content-brand transition-colors shrink-0">House</Link>
 ...
 className="text-sm font-medium text-content-brand hover:text-content-brand transition-colors">
 {product.brand}
 </Link>

 {/* Title */}
 <h1 className="text-xl sm:text-2xl font-bold text-content-primary leading-tight -mt-4">
 {product.title}
 </h1>
 ...
 <Star key={i} className={`w-4 h-4 ${i < Math.floor(product.rating) ? 'fill-amber-400 text-warning' : 'text-content-secondary'}`} />
 ))}
 </div>
 <span className="text-sm font-medium text-content-primary">{product.rating}</span>
 <span className="text-sm text-content-disabled">({product.reviewCount} reviews)</span>
 <span className="text-content-secondary">|</span>
 <span className="text-sm text-content-tertiary">SKU: {product.sku}</span>
 </div>

 {/* Price */}
 <div className="bg-surface-subtle rounded-xl p-4 border border-border">
 <div className="flex items-baseline gap-3">
 <span className="text-3xl font-bold text-content-primary tabular-nums">{formatBDT(currentPrice)}</span>
 {currentCompareAt && (
 ...
 <p className="text-xs text-content-tertiary mt-1">Price inclusive of all taxes</p>
 </div>
 ...
 {optionName}: <span className="text-content-tertiary">{selectedVariant?.optionValues[optionName]}</span>
 </p>
 ...
 ${isSelected
 ? 'border-border-brand bg-surface-brand-subtle text-content-brand shadow-focus'
 : 'border-border text-content-secondary hover:border-neutral-300'
 }
 ...
 <ShareNetwork className="w-4 h-4" /> ShareNetwork
 </button>
 </div>
 ...
 <div>
 <p className="font-medium text-content-primary">Standard Delivery: ৳60</p>
 <p className="text-content-tertiary text-xs">Estimated 3-5 business days</p>
 </div>
 </div>
 <div className="flex items-center gap-3 text-sm">
 <CreditCard className="w-4 h-4 text-content-brand shrink-0" />
 <span className="text-content-secondary">Cash on Delivery available</span>
 </div>
 </div>
 ...
 {[
 { icon: ShieldCheck, text: 'Authentic Product', color: 'text-success' },
 { icon: ArrowCounterClockwise, text: product.returnPolicy, color: 'text-content-brand' },
 ...(product.warranty !== 'N/A' ? [{ icon: ShieldCheck, text: product.warranty, color: 'text-content-brand' }] : []),
 { icon: CreditCard, text: 'Secure Payment', color: 'text-success' },
 ...
 {/* Highlights */}
 <div className="bg-surface-brand-subtle/50 rounded-xl p-5 mb-6">
 <h3 className="text-sm font-semibold text-content-primary mb-3">Key Highlights</h3>
 ...
 <p className="text-sm text-content-secondary leading-relaxed">{product.description}</p>
 </div>
 )}
 ...
 <td className="px-4 py-3 text-sm font-medium text-content-secondary w-1/3">{key}</td>
 <td className="px-4 py-3 text-sm text-content-primary">{value}</td>
 </tr>
 ))}
 </tbody>
 </table>
 ...
 <span className="text-xs font-semibold text-content-brand">{review.name[0]}</span>
 </div>
 <div>
 <p className="text-sm font-medium text-content-primary">{review.name}</p>
 <div className="flex items-center gap-1">
 {Array.from({ length: 5 }).map((_, j) => (
 <Star key={j} className={`w-3 h-3 ${j < review.rating ? 'fill-amber-400 text-warning' : 'text-content-secondary'}`} />
 ))}
 </div>
 </div>
 </div>
 ...
 {discount && <p className="text-xs text-success font-medium">-{discount}% off</p>}
 </div>
 ...
 <Link href="/cart" className="text-sm font-medium text-content-brand hover:text-content-brand shrink-0">
 View Cart
 </Link>
 </div>
 )}
